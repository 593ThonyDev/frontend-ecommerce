...
export const getAllProducts = async (currentPage: number, setIsLoading: (value: boolean) => void): Promise<{ content: ArticleDto[], totalElements: number }> => {
    try {
        ...
        const response: AxiosResponse<{ content: ArticleDto[], totalElements: number }> = await axios.get(API_URL + 'blog/articles', {
            params: {
                page: currentPage,
                size: 12
            }
...
export const getArticleById = async (id: number): Promise<Article> => {
    try {
        setToken();
        const response = await axios.get(`${API_URL}blog/article/${id}`);
